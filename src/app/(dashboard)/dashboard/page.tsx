...
import { Search, KanbanSquare, FileText, TrendingUp, TrendingDown, Link2, Bell, ArrowUpRight } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <Link href="/keywords"><Button size="sm" className="gap-2"><Search className="h-4 w-4" />Voir les mots-clés</Button></Link>
        <Link href="/traffic"><Button size="sm" variant="outline" className="gap-2"><TrendingUp className="h-4 w-4" />Trafic SEO</Button></Link>
        <Link href="/content/new"><Button size="sm" variant="outline" className="gap-2"><FileText className="h-4 w-4" />Nouvel article</Button></Link>
        <Link href="/projects"><Button size="sm" variant="outline" className="gap-2"><KanbanSquare className="h-4 w-4" />Tâches</Button></Link>
      </div>

      <div className="grid gap-6 lg:grid-cols-5">
        <Card className="lg:col-span-3">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-base font-semibold">Top mots-clés</CardTitle>
            <Link href="/keywords"><Button variant="ghost" size="sm" className="gap-1 text-xs text-muted-foreground">Voir tout <ArrowUpRight className="h-3 w-3" /></Button></Link>
          </CardHeader>
          <CardContent>
              ...
              {[
                ...
                <div key={kw.keyword} className="flex items-center justify-between rounded-lg border border-border/50 bg-muted/20 px-4 py-3">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/10 font-mono text-sm font-bold text-primary">{kw.position}</span>
                    <div>
                  ...
                  {kw.change > 0 && <Badge variant="secondary" className="gap-1 bg-emerald-50 text-emerald-700 border-emerald-200"><TrendingUp className="h-3 w-3" />+{kw.change}</Badge>}
                  {kw.change < 0 && <Badge variant="secondary" className="gap-1 bg-red-50 text-red-700 border-red-200"><TrendingDown className="h-3 w-3" />{kw.change}</Badge>}
                  {kw.change === 0 && <Badge variant="secondary" className="text-muted-foreground">=</Badge>}
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Link href="/alerts"><Button variant="ghost" size="sm" className="gap-1 text-xs text-muted-foreground">Voir tout <ArrowUpRight className="h-3 w-3" /></Button></Link>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex gap-3 rounded-lg border border-border/50 bg-muted/20 px-4 py-3">
                  <div className={`mt-0.5 h-2 w-2 shrink-0 rounded-full ${alert.severity === "critical" ? "bg-red-500" : alert.severity === "warning" ? "bg-amber-500" : "bg-emerald-500"}`} />
                  <div>
